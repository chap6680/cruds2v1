...
If ($_SERVER['REQUEST_METHOD']=='POST') {
        $project_id = trim(filter_input(INPUT_POST, 'project_id', FILTER_SANITIZE_STRING));
        ...
        If (empty($project_id) || empty($title) || empty($date) || empty($time)){
            $error_message = 'Please fill in the Title'.$title.' project -->'.$project_id.'<-and date'.$date.' now time'.$time;
        } else {
